...
def _to_axis_pid(store: List[Dict], t0: float) -> Optional[AxisPIDSignal]:
    ...
    if 0 < max_signal < 6.5:
        ...
        logger.debug(
            "PID data auto-converted rad/s → deg/s " "(max_signal=%.3f rad/s → %.1f deg/s)",
            max_signal,
            max_signal * _RAD2DEG,
        )
...
class ArduPilotAdapter(PlatformAdapter):
    ...
    def parse(self, path: Path) -> FlightData:  # noqa: C901
        ...
        ahr2_acc = _ColAccum(
            ["time", "Roll", "Pitch", "Yaw", "Q1", "Q2", "Q3", "Q4", "Lat", "Lng", "Alt"]
        )
        ...
        try:
            while True:
                ...
                if mtype == "IMU":
                    ...
                    rate_rll_acc.append(
                        time=ts,
                        Des=getattr(msg, "Tar", getattr(msg, "Des", 0.0)),
                        Act=getattr(msg, "Act", 0.0),
                        Err=getattr(msg, "Err", 0.0),
                        P=getattr(msg, "P", 0.0),
                        I=getattr(msg, "I", 0.0),
                    ...
                    rdes = getattr(msg, "RDes", 0.0)
                    r = getattr(msg, "R", 0.0)
                    pdes = getattr(msg, "PDes", 0.0)
                    p = getattr(msg, "P", 0.0)
                    ydes = getattr(msg, "YDes", 0.0)
                    y = getattr(msg, "Y", 0.0)
                    rate_rll_leg.append(
                        time=ts, Des=rdes, Act=r, Err=rdes - r, P=0.0, I=0.0, D=0.0, Limit=0
                    )
                    rate_pit_leg.append(
                        time=ts, Des=pdes, Act=p, Err=pdes - p, P=0.0, I=0.0, D=0.0, Limit=0
                    )
                    rate_yaw_leg.append(
                        time=ts, Des=ydes, Act=y, Err=ydes - y, P=0.0, I=0.0, D=0.0, Limit=0
                    )

                elif mtype == "COMPASS":
                    compass_acc.append(
                        compass_id=getattr(msg, "I", 0),
                        time=ts,
                        MagX=getattr(msg, "MagX", 0.0),
                        MagY=getattr(msg, "MagY", 0.0),
                        MagZ=getattr(msg, "MagZ", 0.0),
                        OfsX=getattr(msg, "OfsX", 0.0),
                        OfsY=getattr(msg, "OfsY", 0.0),
                        OfsZ=getattr(msg, "OfsZ", 0.0),
                        MOfsX=getattr(msg, "MOfsX", 0.0),
                        MOfsY=getattr(msg, "MOfsY", 0.0),
                        MOfsZ=getattr(msg, "MOfsZ", 0.0),
                    )
                    ...
                    compass_acc.append(
                        ...
                        OfsY=getattr(msg, "OfsY", 0.0),
                        OfsZ=getattr(msg, "OfsZ", 0.0),
                        MOfsX=getattr(msg, "MOX", 0.0),
                        MOfsY=getattr(msg, "MOY", 0.0),
                        MOfsZ=getattr(msg, "MOZ", 0.0),
                    )
                    ...
                    ahr2_acc.append(
                        ...
                        Pitch=getattr(msg, "Pitch", 0.0),
                        Yaw=getattr(msg, "Yaw", 0.0),
                        Q1=getattr(msg, "Q1", 1.0),
                        Q2=getattr(msg, "Q2", 0.0),
                        Q3=getattr(msg, "Q3", 0.0),
                        Q4=getattr(msg, "Q4", 0.0),
                        Lat=getattr(msg, "Lat", 0.0),
                        Lng=getattr(msg, "Lng", 0.0),
                        Alt=getattr(msg, "Alt", 0.0),
                    )
                    ...
                    pos.append(
                        {
                            ...
                            "Lng": getattr(msg, "Lng", 0.0),
                            "Alt": getattr(msg, "Alt", 0.0),
                            "RelHomeAlt": getattr(msg, "RelHomeAlt", 0.0),
                            "RelOriginAlt": getattr(msg, "RelOriginAlt", 0.0),
                        }
                    )
                    ...
                    bat_acc.append(
                        time=ts,
                        I=getattr(msg, "I", 0),
                        Volt=getattr(msg, "Volt", 0.0),
                        Curr=getattr(msg, "Curr", 0.0),
                        CurrTot=getattr(msg, "CurrTot", 0.0),
                        EnrgTot=getattr(msg, "EnrgTot", 0.0),
                        Temp=getattr(msg, "Temp", 0.0),
                        Res=getattr(msg, "Res", 0.0),
                    )

                elif mtype == "PARM":
                    raw_name = getattr(msg, "Name", b"")
                    name = (
                        raw_name.decode("utf-8", errors="replace").strip("\x00")
                        if isinstance(raw_name, bytes)
                        else str(raw_name).strip("\x00")
                    )
                    try:
                    ...
                    ver = {
                        "time": ts,
                        "FWVer": getattr(msg, "FWVer", ""),
                        "APJ": getattr(msg, "APJ", 0),
                        "GH": getattr(msg, "GH", ""),
                        "FV": getattr(msg, "FV", 0),
                    }
                    ...
                    atde.append(
                        {
                            "time": ts,
                            "axis": getattr(msg, "Axis", -1),
                            "step": getattr(msg, "Step", -1),
                            "wave": getattr(msg, "Wave", -1),
                            "rate": getattr(msg, "Rate", 0.0),
                            "P": getattr(msg, "P", 0.0),
                            "I": getattr(msg, "I", 0.0),
                            "D": getattr(msg, "D", 0.0),
                        }
                ...
                compass_list.append(
                    {
                        ...
                        "MagX": compass_d["MagX"][i],
                        "MagY": compass_d["MagY"][i],
                        "MagZ": compass_d["MagZ"][i],
                        "OfsX": compass_d["OfsX"][i],
                        "OfsY": compass_d["OfsY"][i],
                        "OfsZ": compass_d["OfsZ"][i],
                        "MOfsX": compass_d["MOfsX"][i],
                        "MOfsY": compass_d["MOfsY"][i],
                        "MOfsZ": compass_d["MOfsZ"][i],
                    }
                )
